 ...
 * @param <ResultT>
 */
public abstract class SafeAsyncTask<Progress,ResultT> implements Callable<ResultT> {
	...
	public FutureTask<Void> future() {
        task=newTask();
		future = new FutureTask<Void>(task);
		return future;
	}

	public SafeAsyncTask<Progress,ResultT> executor(Executor executor) {
		this.executor = executor;
		return this;
	}
	...
	public SafeAsyncTask<Progress,ResultT> handler(Handler handler) {
		this.handler = handler;
		return this;
	}
    ...
    public void publishProgress (final Progress... progress) throws Exception {
        if (future == null)
            throw new UnsupportedOperationException(
                    "You cannot cancel this task before calling future()");
    ...
    protected void onProgressUpdate(Progress... t) throws Exception {
    }
	/**
	...
	protected Task<Progress,ResultT> newTask() {
		return new Task<Progress,ResultT>(this);
	}

	public static class Task<Progress,ResultT> implements Callable<Void> {
		protected final SafeAsyncTask<Progress,ResultT> parent;
		protected final Handler handler;

		public Task(SafeAsyncTask<Progress,ResultT> parent) {
			this.parent = parent;
		...
		public Void call() throws Exception {
			try {
				...
				try {
					...
					// logged but ignored
					Log.e("SafeAsyncTask", f.getMessage());
				}
        ...
        protected void doProgress(final Progress... progress) throws Exception {
            postToUiThreadAndWait(new Callable<Object>() {
                @Override
                public Object call() throws Exception {
                    parent.onProgressUpdate(progress);
                    return null;
                }
            });
        }
